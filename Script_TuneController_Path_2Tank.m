...
model_params.decoupling.decoupling_type = "Simplified"; % Ignored if activate_dec = 0
model_params.valve_aperture = valve_aperture;  % coupling valve aperture \in [0, 1]
model_params.decoupling.decoupling_valve_aperture = valve_aperture;
model_params.decoupling.decoupling_tf = tf(eye(2));  % placeholder
% Get model struct
Config = set_up_tuning_config(model_name, stages, model_params);
Config.ss_model = ss_model;
