function plot_4tank_step(sim_out, var_plot, plot_group, save_folder)
...
%                 save_folder = " " if figure won't be saved.
    ...
    if plot_group == "2x2"
        if var_plot(1) == 1
            ...
            % X3
            subplot(2,2,1)
            plot(state_time, state_data(:, 3), "LineWidth", meas_linewidth)
            % xline(waypoint_changes, "LineWidth", vline_linewidth)
            set(gca, "Fontsize", ax_fontsize)
            title('Tank 3', FontSize=title_fontsize)
            xlabel('Time[s]', FontSize=xlabel_fontsize)
            ylabel('Level [cm]', FontSize=ylabel_fontsize)
            % Y
            subplot(2,2,2)
            plot(state_time, state_data(:, 4), "LineWidth", meas_linewidth)
            % xline(waypoint_changes, "LineWidth", vline_linewidth)
            set(gca, "Fontsize", ax_fontsize)
            title('Tank 4', FontSize=title_fontsize)
            xlabel('Time[s]', FontSize=xlabel_fontsize)
            ylabel('Level [cm]', FontSize=ylabel_fontsize)
            % Z
            subplot(2,2,3)
